...
data_dir = os.path.join(workspace, "data_test")
...
def build_dataset(
    ...
    **kwargs,
    ...
    dataset = TestDataSet(data_dir, name, **dataset_config)
...
def main_test_process(args, cfg):
    ...
    case_name, ext = os.path.splitext(basename)
    # init env
    ...
    # save_checkpoint(network,'train_data_test/abinet.ckpt')
    ...
    num_epoch = 3 if cfg.model.head.name != "DBHead" else 1
    base_lr_dict = {
    ...
    # lr_scheduler=[cfg.scheduler.lr for _ in range(200)]
    ...
    average_step_time = (train_end_time - first_step_end_time) / (num_epoch * 100 - 1) * 1000
...
def test_db_r50_1p():
    ...
    """
    args, config = parse_args_and_config([f"--config={workspace}/configs/det/dbnet/db_r50_icdar15.yaml"])
    config = Dict(config)

    loss_start, loss_end, _, _ = main_test_process(args, config)

    assert 35.55 <= loss_start <= 35.65, f"Loss start should in [32.90, 33.00], but got {loss_start}"
    assert 10.09 <= loss_end <= 10.19, f"Loss end should in [10.09, 10.19], but got {loss_end}"
...
def test_crnn_vgg7_1p():
    ...
    Expectation: No exception.
    """
    args, config = parse_args_and_config([f"--config={workspace}/configs/rec/crnn/crnn_vgg7.yaml"])
    config = Dict(config)

    loss_start, loss_end, _, _ = main_test_process(args, config)

    assert 71.89 <= loss_start <= 71.99, f"Loss start should in [71.89, 71.99], but got {loss_start}"
    assert 0.29 <= loss_end <= 0.39, f"Loss end should in [0.29, 0.39], but got {loss_end}"
