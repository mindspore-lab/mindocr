...
BOXES_2 = [
    [[30, 30], [30, 130], [70, 130], [70, 30]],
    [[100, 200], [100, 350], [250, 250], [250, 200]],
]


def _gen_text_image(texts=TEXTS_2, boxes=BOXES_2, save_fp="gen_img.jpg"):
...
def test_det_infer():
    algo = "DB"
    cmd = (
        f"python tools/infer/text/predict_det.py --image_dir {det_img_fp} --det_algorithm {algo} "
...
def test_rec_infer():
    algo = "CRNN"
    cmd = (
        f"python tools/infer/text/predict_rec.py --image_dir {rec_img_fp} --rec_algorithm {algo} "
