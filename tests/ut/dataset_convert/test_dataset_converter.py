"""
The test ensures that the method correctly formats the label according to the input image directory and produces the expected output file.
This test sets up a temporary test directory, creates test label and image files, initializes an testing for ic15 converter.
...
class TestDatasetPaths(object):
    ...
    def checkIC15(self):
        ...
        for dirname in ['ch4_test_images', 'ch4_test_vocabularies_per_image',
                        'ch4_training_images', 'ch4_training_vocabularies_per_image',
                        'ch4_test_word_images_gt', 'ch4_training_word_images_gt',
                        'ch4_training_localization_transcription_gt',
                        'Challenge4_Test_Task4_GT',]:
            subfolder = os.path.join(dataset_dir, dirname)
            self.checkDirValidity(subfolder)
        
        for filename in ['ch4_test_vocabulary.txt', 'ch4_training_vocabulary.txt', 'GenericVocabulary.txt', 'Challenge4_Test_Task3_GT.txt']:
...
class TestConverterIC15(unittest.TestCase):
    def setUp(self):
        ...
        with open(label_file, 'w') as f:
            f.write('377,117,463,117,465,130,378,130,Genaxis Theatre\n' # 8 coordinates: x1, y1, x2, y2, x3, y3, x4, y4, transcription
    ...
    def _test_format_det_label(self):
        ...
        with open(output_file, 'r') as f:
            contents = f.read()
        expected_contents = ('img1.jpg\t[{"transcription": "Genaxis Theatre", "points": [[377, 117], [463, 117], [465, 130], [378, 130]]}, {"transcription": "###", "points": [[374, 155], [409, 155], [409, 170], [374, 170]]}]\n' 
                             'img2.jpg\t[{"transcription": "EXIT", "points": [[602, 173], [635, 175], [634, 197], [602, 196]]}, {"transcription": "I2R", "points": [[734, 310], [792, 320], [792, 364], [738, 361]]}]\n')
...
class TestConverterTOTALTEXT(unittest.TestCase):
    def setUp(self):
        ...
        with open(label_file, 'w') as f:
            f.write("x: [[206 251 386 542 620 646 550 358 189 140]], y: [[ 633  811  931  946  926  976 1009  989  845  629]], ornt: [u'c'], transcriptions: [u'PETROSAINS']\n") # 20 coordinates, orientations, transcription
        label_file = os.path.join(annot_dir, 'poly_gt_img2.txt')
        with open(label_file, 'w') as f:
            f.write("x: [[115 503 494 115]], y: [[322 346 426 404]], ornt: [u'm'], transcriptions: [u'nauGHTY']\n" # 8 coordinates, orientations, transcription
    ...
    def _test_format_det_label(self):
        ...
        with open(output_file, 'r') as f:
            contents = f.read()
        expected_contents = ('img1.jpg\t[{"transcription": "PETROSAINS", "points": [[206, 633], [251, 811], [386, 931], [542, 946], [620, 926], [646, 976], [550, 1009], [358, 989], [189, 845], [140, 629]]}]\n' 
                             'img2.jpg\t[{"transcription": "nauGHTY", "points": [[115, 322], [503, 346], [494, 426], [115, 404]]}, {"transcription": "NURIS", "points": [[734, 360], [1058, 369], [1061, 449], [744, 430]]}]\n')
        self.assertEqual(contents, expected_contents)
...
class TestConverterCOCOFORMAT(unittest.TestCase):
    def setUp(self):
        ...
        # Create label and image files for testing
        label_img1 = [{'image_id': 60001, 'bbox': [218.0, 406.0, 138.0, 47.0], 'area': 6486.0, 'rec': [95, 95, 95, 95, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96], 'category_id': 1, 'iscrowd': 0, 'id': 1, 'bezier_pts': [219.0, 406.0, 264.33, 407.0, 309.67, 408.0, 355.0, 409.0, 352.0, 448.0, 307.33, 449.33, 262.67, 450.67, 218.0, 452.0]},
                     ]
        data_img1 = {'width': 400, 'date_captured': '', 'license': 0, 'flickr_url': '', 'file_name': '0060001.jpg', 'id': 60001, 'coco_url': '', 'height': 600}
        label_img2 = [ {'image_id': 60002, 'bbox': [335.0, 81.0, 252.0, 110.0], 'area': 27720.0, 'rec': [95, 95, 95, 95, 95, 95, 95, 95, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96], 'category_id': 1, 'iscrowd': 0, 'id': 6, 'bezier_pts': [341.0, 81.0, 422.67, 98.33, 504.33, 115.67, 586.0, 133.0, 582.0, 190.0, 499.67, 172.67, 417.33, 155.33, 335.0, 138.0]},
                     {'image_id': 60002, 'bbox': [69.0, 182.0, 17.0, 19.0], 'area': 323.0, 'rec': [20, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96, 96], 'category_id': 1, 'iscrowd': 0, 'id': 7, 'bezier_pts': [70.0, 183.0, 75.0, 182.67, 80.0, 182.33, 85.0, 182.0, 84.0, 199.0, 79.0, 199.33, 74.0, 199.67, 69.0, 200.0]}]
        data_img2 = {'width': 400, 'date_captured': '', 'license': 0, 'flickr_url': '', 'file_name': '0060002.jpg', 'id': 60002, 'coco_url': '', 'height': 600}
        label_file = {'licenses': [], 'info':{}, 'categories': [{'supercategory': 'beverage', 'id': 1, 'keypoints': ['mean', 'xmin', 'x2', 'x3', 'xmax', 'ymin', 'y2', 'y3', 'ymax', 'cross'], 'name': 'text'}]}
    ...
    def _test_format_det_label(self):
        ...
        with open(output_file, 'r') as f:
            contents = f.read()
        expected_contents = ('0060001.jpg\t[{"transcription": "口口口口", "points": [[218, 406], [356, 406], [356, 453], [218, 453]], "bezier": [219.0, 406.0, 264.33, 407.0, 309.67, 408.0, 355.0, 409.0, 352.0, 448.0, 307.33, 449.33, 262.67, 450.67, 218.0, 452.0]}]\n' 
                             '0060002.jpg\t[{"transcription": "口口口口口口口口", "points": [[335, 81], [587, 81], [587, 191], [335, 191]], "bezier": [341.0, 81.0, 422.67, 98.33, 504.33, 115.67, 586.0, 133.0, 582.0, 190.0, 499.67, 172.67, 417.33, 155.33, 335.0, 138.0]}, {"transcription": "4", "points": [[69, 182], [86, 182], [86, 201], [69, 201]], "bezier": [70.0, 183.0, 75.0, 182.67, 80.0, 182.33, 85.0, 182.0, 84.0, 199.0, 79.0, 199.33, 74.0, 199.67, 69.0, 200.0]}]\n')
        self.assertEqual(contents, expected_contents)
