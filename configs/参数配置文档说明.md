...
详情请看 [mindocr/optim](../mindocr/optim)
...
训练流程的配置放在 `train` 底下，评估阶段的配置放在 `eval` 底下。注意，在模型训练的时候，若打开边训练边评估模式，即val_while_train=True，则在每个epoch训练完毕后安装 `eval` 底下的配置运行一次评估。在非训练阶段，只运行模型评估的时候，会只读取 `eval` 配置。
