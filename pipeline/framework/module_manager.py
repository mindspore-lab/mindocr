...
from pipeline.datatype.module_data import ModuleInitArgs, ModulesInfo
from pipeline.utils import log
from mindocr.infer.node_config import processor_initiator
...
class ModuleManager:
    ...
    @staticmethod
    def init_module_instance(module_instance, instance_id, pipeline_name, module_type, module_name):
    ...
    def register_modules(self, pipeline_name: str, module_desc_list: list, default_count: int):
        log.info("----------------------------------------------------")
        log.info("---------------register_modules start---------------")
        modules_info_dict = self.pipeline_map[pipeline_name]
        self.pipeline_name = pipeline_name

        for i, module_desc in enumerate(module_desc_list):
            log.info("+++++++++++++++++++++++++++++++++++++")
            ...
            for instance_id in range(module_count):
                if i == 0:  # HandoutNode
                    tqdm_info = {"i": i, "queue_len": self.task_queue._maxsize}
                else:
                    tqdm_info = {"i": i, "queue_len": self.MODULE_QUEUE_MAX_SIZE}
                module_instance = processor_initiator(module_desc.module_type)(self.args, self.msg_queue, tqdm_info)
                self.init_module_instance(module_instance,
                                          instance_id,
                                          pipeline_name,
        ...
        log.info("----------------register_modules end---------------")
        log.info("----------------------------------------------------")

    def register_module_connects(self, pipeline_name: str, connect_desc_list: list):
