...
class LayoutAnalyzer(object):
    ...
    def __call__(self, image: str, do_visualize: bool = False) -> List:
        """
            Args:
        image: a raw input image as a numpy array
    ...
    def _preprocess(self, image: np.ndarray) -> np.ndarray:
        """
        Data preprocess for layout analysis
        """
        # resize image
        ...
        # data = run_transforms(data, transform_pipeline)
    ...
    def _postprocess(self, preds: np.ndarray) -> Dict:
        """
        Data postprocess for layout analysis
        """
        # postprocess
        postprocess_ops = YOLOv8Postprocess(conf_thres=0.5, iou_thres=0.7, conf_free=True)
        results = postprocess_ops(preds, [self.img_shape], meta_info=([0], [self.hw_ori], [self.hw_scale], [self.pad]))
        return results
...
if __name__ == "__main__":
    ...
    for i, img_path in enumerate(img_paths):
        ...
        layout_res = layout_analyzer(image, do_visualize=args.visualize_output)
