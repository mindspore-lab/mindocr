...
class Predict_System:
    def __init__(self, det_cfg, rec_cfg, is_concat):
    ...
    def predict_rec(self, crops: list):
        ...
        for idx in range(0, num_crops, self.rec_batch_size):  # batch begin index i
            ...
            for j in range(batch_begin, batch_end):  # image index j
                ...
                cv2.imwrite(f"crop_{j}.png", data["image"].transpose(1, 2, 0))
...
def create_parser():
    ...
    parser.add_argument("--is_concat", type=str, help="image path or image directory")
    parser.add_argument(
        "-o",
        "--opt",
        nargs="+",
