...
report_paddle2onnx
CONFIG_FILE=$ppocr_path/models/${rec_model}/static_config.txt
DATA_SHAPE_H=48
DATA_SHAPE_W=320
generate_static_shape_config_file
SAVE_REC_MINDIR_FILE=$ppocr_path/models/${rec_model}/rec_crnn_static_output
cmd="$CONVERTER_PATH --saveType=MINDIR --fmk=ONNX --optimize=ascend_oriented --modelFile=$SAVE_REC_ONNX_FILE --outputFile=$SAVE_REC_MINDIR_FILE --configFile=static_config.txt $infoCmd"
echo -e "\033[36mconvert command:\033[0m $cmd" | tee -a $logFile
eval $cmd
SAVE_CONVERT_FILE=$SAVE_REC_MINDIR_FILE
report_convert
mv $ppocr_path/models/${rec_model}/rec_crnn_static_output.mindir $ppocr_path/${PPOCR_MODEL_NAME}_rec_crnn_static_output.mindir
...
report_paddle2onnx
CONFIG_FILE=$ppocr_path/models/${cls_model}/static_config.txt
DATA_SHAPE_H=48
DATA_SHAPE_W=192
generate_static_shape_config_file
SAVE_CLS_MINDIR_FILE=$ppocr_path/models/${cls_model}/cls_mv4_static_output
cmd="$CONVERTER_PATH --saveType=MINDIR --fmk=ONNX --optimize=ascend_oriented --modelFile=$SAVE_CLS_ONNX_FILE --outputFile=$SAVE_CLS_MINDIR_FILE --configFile=static_config.txt $infoCmd"
echo -e "\033[36mconvert command:\033[0m $cmd" | tee -a $logFile
eval $cmd
SAVE_CONVERT_FILE=$SAVE_CLS_MINDIR_FILE
report_convert
mv $ppocr_path/models/${cls_model}/cls_mv4_static_output.mindir $ppocr_path/${PPOCR_MODEL_NAME}_cls_mv4_static_output.mindir
