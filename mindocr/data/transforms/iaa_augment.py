        ...
        if "polys" in data:
            if self.is_train:
                ...
                for poly in data["polys"]:
                    kps = imgaug.KeypointsOnImage(
                        [imgaug.Keypoint(p[0], p[1]) for p in poly], shape=data["image"].shape
                    )
                ...
                raise ValueError(
                    "Test time augmentation is not supported for detection currently (due to transformed polygons "
                    "can be not recoveried for evaluation). IaaAugment should only be used for training."
                )
