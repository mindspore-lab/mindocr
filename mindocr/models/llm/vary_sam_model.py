...
class Attention(nn.Cell):
    ...
    def __init__(self,
        ...
        self.qkv = Linear(in_channels=dim,
                          out_channels=dim * 3,
                          has_bias=qkv_bias,
                          compute_dtype=compute_dtype,
                          param_init_type=param_init_type)
        self.proj = Linear(in_channels=dim,
                           out_channels=dim,
                           compute_dtype=compute_dtype,
                           param_init_type=param_init_type)
...
def window_partition(x: ms.Tensor,
                     window_size: int,
                     pad_size: Tuple[int, int] = 0) -> Tuple[ms.Tensor, Tuple[int, int]]:
    """
...
def window_unpartition(windows: ms.Tensor,
                       window_size: int,
                       pad_hw: Tuple[int, int],
                       hw: Tuple[int, int]) -> ms.Tensor:
    """
...
def add_decomposed_rel_pos(attn: ms.Tensor,
                           q: ms.Tensor,
                           rel_pos_h: ms.Tensor,
                           rel_pos_w: ms.Tensor,
                           q_size: Tuple[int, int],
                           k_size: Tuple[int, int]) -> ms.Tensor:
    """
    ...
    attn = (attn.view(b, q_h, q_w, k_h, k_w) + rel_h[:, :, :, :, None] + rel_w[:, :, :, None, :]).view(b, q_h * q_w,
                                                                                                       k_h * k_w)
