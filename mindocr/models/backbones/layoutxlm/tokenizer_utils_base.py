...
from mindspore import __version__

from .downloader_utils import get_path_from_url_with_filelock, url_file_exists
from .utils import resolve_cache_dir

logger = logging.getLogger("mindocr.layoutxlm")
...
def to_py_obj(obj):
    """
    Convert a MS tensor, Numpy array or python list to a python list.
    """
...
class BatchEncoding(UserDict):
    ...
    Args:
        ...
        tensor_type (`Union[None, str, TensorType]`, *optional*):
            You can give a tensor_type here to convert the lists of integers in Tensors at
            initialization.
...
class PretrainedTokenizerBase(SpecialTokensMixin):
    ...
    Class attributes (overridden by derived classes)
         ...
         - **resource_files_names** (`Dict[str, str]`) -- A dictionary with, as keys, the `__init__` keyword name of
         each vocabulary file required by the model, and as associated values,
         the filename for saving the associated file(string).
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"truncation_side='{self.truncation_side}', "
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, from_hf_hub=False, subfolder=None, **kwargs):
        ...
        # From local dir path
            ...
            for file_id, file_name in vocab_files_target.items():
                full_file_name = "/".join([pretrained_model_name_or_path, file_name])
                vocab_files[file_id] = full_file_name
        ...
        # TODO: It's not supportted to load tokenizer.json of hf so far.
        if from_hf_hub and "tokenizer_file" in init_kwargs:
            init_kwargs.pop("tokenizer_file")

        # TODO: avoid reduplication of position args and key word args
    ...
    def save_to_hf_hub(
        ...
        Args:
            ...
            commit_message (str, optional) — The summary / title / first line of the generated commit.
             Defaults to: f"Upload {path_in_repo} with huggingface_hub"
            revision (str, optional) — The git revision to commit from. Defaults to the head of the "main" branch.
            create_pr (boolean, optional) — Whether or not to create a Pull Request with that commit. Defaults to False.
                If revision is not set, PR is opened against the "main" branch.
                If revision is set and is a branch, PR is opened against this branch.
                If revision is set and is not a branch name (example: a commit oid),
                an RevisionNotFoundError is returned by the server.
                ...
                if self.model_max_length > LARGE_INTEGER:
                    if verbose:
                        if not self.deprecation_warnings.get("Asking-to-pad-to-max_length", False):
                            warnings.warn(
                                "Asking to pad to max_length but no maximum "
                                "length is provided and the model has no predefined maximum length. "
                    ...
                    if verbose:
                        if not self.deprecation_warnings.get("Asking-to-truncate-to-max_length", False):
                            warnings.warn(
                                ...
                                "is provided and the model has no predefined maximum length. "
    ...
    def __call__(
        ...
        Args:
            ...
            is_split_into_words (Union[bool, str], optional):
                ...
                `token`: means that the text should be tokens which already be tokenized,
                so it should not be tokenized again.
            ...
            return_tensors (str or [TensorType], optional):
                ...
                - `'pd'`: Return MS `ms.Tensor` objects.
        ...
        if is_batched:
            if isinstance(text_pair, str):
                raise TypeError(
                    "when tokenizing batches of text, "
                    "`text_pair` must be a list or tuple with the same length as `text`."
                )
    ...
    def batch_encode(
            ...
            truncation: Union[bool, str, TruncationStrategy] = False,
            return_position_ids=False,
            # TODO: keep align with `encode` method
    ...
    def pad(
        ...
        If the `encoded_inputs` passed are dictionary of numpy arrays, MindSpore tensors, the
        result will use the same type unless you provide a different tensor type with `return_tensors`.
        </Tip>

        Args:
            encoded_inputs ([`BatchEncoding`], list of [`BatchEncoding`], `Dict[str, List[int]]`,
            `Dict[str, List[List[int]]` or `List[Dict[str, List[int]]]`):
                ...
                Instead of `List[int]` you can have tensors (numpy arrays, MindSpore tensors), see
            ...
            return_tensors (`str` or [`TensorType`], *optional*):
                ...
                - `'pd'`: Return MindSpore `ms.Tensor` objects.
                - `'np'`: Return Numpy `np.ndarray` objects.
        ...
        # If we have MindSpore/NumPy tensors/arrays as inputs, we cast them as python objects
        ...
        if not isinstance(first_element, (int, list, tuple)):
            if isinstance(first_element, ms.Tensor):
                ...
                raise ValueError(
                    ...
                    f"Should be either python or MindSpore object."
                )
    ...
    def clean_up_tokenization(out_string: str) -> str:
        ...
        """
        out_string = (
            out_string.replace(" .", ".").replace(" ?", "?").replace(" !", "!").replace(" ,", ",").replace(" ' ",
                                                                                                           "'").replace(
                " n't", "n't").replace(" 'm", "'m").replace(" 's", "'s").replace(" 've", "'ve").replace(" 're", "'re")
        )
        return out_string
