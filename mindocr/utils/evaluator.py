...
class Evaluator:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.metric_names = []
        for m in metrics:
            assert hasattr(m, "metric_names") and isinstance(
                m.metric_names, List
            ), f"Metric object must contain `metric_names` attribute to indicate the metric names as a List type, but not found in {m.__class__.__name__}"
