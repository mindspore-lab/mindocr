...
class CTCLoss(LossBase):
    """
     ...
     Args:
        ...
        max_label_length(int): the maximum number of characters in a text label, i.e. max_text_length in yaml.
        batch_size(int): batch size of input logits. bs
     """

    def __init__(self, pred_seq_len=26, max_label_length=25, batch_size=32):
        super(CTCLoss, self).__init__()
        assert pred_seq_len > max_label_length, 'pred_seq_len is required to be larger than max_label_length for CTCLoss. Please adjust the strides in the backbone, or reduce max_text_length in yaml'
