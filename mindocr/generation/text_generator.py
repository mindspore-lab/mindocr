...
from mindocr.generation.beam_search import BeamSearchScorer
from mindocr.generation.generation_config import GenerationConfig
from mindocr.generation.logits_process import (LogitNormalization, LogitsProcessorList,
                                               RepetitionPenaltyLogitsProcessor,
                                               TemperatureLogitsWarper, TopKLogitsWarper,
                                               TopPLogitsWarper)
from mindocr.generation.utils import softmax_with_threads, topk
...
class GeneratorMixin:
    ...
    def _greedy_search(self,
        ...
        if generation_config.max_length > self.config.seq_length:
            _logger.warning("max_length %s can not exceeds model seq_length %s, set max_length = seq_length.",
        ...
        while np.sum(is_finished) != batch_size:
            ...
            _logger.debug("forward time: %s s; greedy search time: %s s; update time: %s s; total count: %s s",
        ...
        _logger.info("total time: %s s; generated tokens: %s tokens; generate speed: %s tokens/s",
        ...
        while np.sum(is_finished) != batch_size:
            ...
            if streamer is not None:
                if batch_size == 1:
                    ...
                    streamer.put(target_list)
            update_time = time.time() - update_time
            _logger.debug("forward time: %s s; sample time: %s s; update time: %s s; total count: %s s",
    ...
    def _beam_search(self,
        ...
        while True:
            ...
            if is_encoder_decoder:
                ...
                # incremental generate
                if generation_config.use_past:
                    _logger.warning("Beam search currently not support incremental, "
            ...
            update_time = time.time() - update_time
            _logger.debug("forward time: %s s; beam search time: %s s; update time: %s s; total count: %s s",
                          forward_time, search_time, update_time, forward_time + search_time + update_time)
    ...
    def generate(self,
        ...
        if generation_config.num_beams > 1:
            _logger.warning("When num_beams is set to a value greater than 1, do_sample will be set to False, "
                            "due to the current beam search does not support sampling.")
            generation_config.do_sample = False
        if not generation_config.do_sample:
            _logger.warning("When do_sample is set to False, top_k will be set to 1 and top_p will be set to 0, "
