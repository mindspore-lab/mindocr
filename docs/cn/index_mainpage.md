...
> 注意：
> - 如果使用ACL推理(请见: 2.1 使用mindspore lite和acl推理)，Python版本需为3.9。
...
MindOCR支持多种文本检测模型及数据集，在此我们使用**DBNet**模型和**ICDAR2015**数据集进行演示。请参考`configs/det/dbnet/README_CN.md`。


### 1.2 文本识别

MindOCR支持多种文本识别模型及数据集，在此我们使用**CRNN**模型和**LMDB**数据集进行演示。请参考`configs/rec/crnn/README_CN.md`。
...
模型训练的配置及性能结果请见`configs/`目录。

MindSpore Lite和ACL模型推理的支持列表，请见[MindOCR模型推理支持列表](inference/inference_models_list.md)和[第三方模型推理支持列表](inference/inference_models_list_thirdparty.md).
...
- [x] ICDAR2015 [论文]((https://rrc.cvc.uab.es/files/short_rrc_2015.pdf)) [主页]((https://rrc.cvc.uab.es/?ch=4))

- [x] Total-Text [论文](https://arxiv.org/abs/1710.10400) [主页](https://github.com/cs-chan/Total-Text-Dataset/tree/master/Dataset)

- [x] Syntext150k [论文](https://arxiv.org/abs/2002.10200) [主页](https://github.com/aim-uofa/AdelaiDet)

- [x] MLT2017 [论文](https://ieeexplore.ieee.org/abstract/document/8270168) [主页](https://rrc.cvc.uab.es/?ch=8&com=introduction)

- [x] MSRA-TD500 [论文](https://ieeexplore.ieee.org/abstract/document/6247787) [主页](http://www.iapr-tc11.org/mediawiki/index.php/MSRA_Text_Detection_500_Database_(MSRA-TD500))

- [x] SCUT-CTW1500 [论文](https://www.sciencedirect.com/science/article/pii/S0031320319300664) [主页](https://github.com/Yuliang-Liu/Curve-Text-Detector)

</details>

### 转换

在 `DATASETS_DIR` 文件夹中下载这些数据集后，您可以运行 `bash tools/convert_datasets.sh` 将所有下载的数据集转换为目标格式。[这里](mkdocs/dataset_converters.md)有一个 icdar2015 数据集转换的例子。
...
1. 增加新模型
    - 文本检测DBNet++
    - 文本识别CRNN-Seq2Seq
    - 在SynthText数据集上预训练的DBNet的[权重文件](https://download.mindspore.cn/toolkits/mindocr/dbnet/dbnet_resnet50_synthtext-40655acb.ckpt)
2. 添加更多基准数据集及其结果
    - [SynthText](https://academictorrents.com/details/2dba9518166cbd141534cbf381aa3e99a087e83c), MSRA-TD500, CTW1500
    - DBNet的更多基准结果请见: `configs/det/dbnet/README_CN.md`.
