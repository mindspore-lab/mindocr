...
from deploy.mindx.data_type import StopData, ProcessData, ProfilingData
from deploy.mindx.framework import ModuleBase, InferModelComb
from deploy.mindx.utils import safe_list_writer, log
...
class CollectProcess(ModuleBase):
    ...
    def single_image_save(self, image_name, image):
        if self.args.save_pipeline_crop_res:
            filename = os.path.join(self.args.pipeline_crop_save_dir, os.path.basename(image_name))
        ...
        if self.args.save_vis_pipeline_save_dir:
            filename = os.path.join(self.args.vis_pipeline_save_dir, os.path.basename(image_name))
        ...
        if self.args.save_vis_det_save_dir:
            filename = os.path.join(self.args.vis_det_save_dir, os.path.basename(image_name))
