...
from .framework import InferEngine, InferModelComb
from .processors import SUPPORT_DET_MODEL, SUPPORT_REC_MODEL
from .utils import log, save_path_init
...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Input images dir for inference, can be dir containing multiple images or path of single " "image.",
    )
    parser.add_argument(
        "--engine",
        type=str.lower,
    ...
    parser.add_argument(
        "--rec_char_dict_path", type=str, required=False, help="Character dict file path for recognition models."
    )
    ...
    parser.add_argument(
        "--pipeline_crop_save_dir", type=str, required=False, help="Saving dir for images cropped during pipeline."
    )
    parser.add_argument(
...
def update_task_args(args):
    ...
    engine_map = {"mindx": InferEngine.MindX, "lite": InferEngine.Lite}
...
def check_args(args):
    ...
    if args.pipeline_crop_save_dir and args.task_type not in (InferModelComb.DET_REC, InferModelComb.DET_CLS_REC):
        raise ValueError("det_model_path and rec_model_path can't be empty when set pipeline_crop_save_dir.")
