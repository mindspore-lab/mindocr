...
def _filter_env_level():
    ...
    if (
        not isinstance(log_env_level, str)
        or not log_env_level.isdigit()
        or int(log_env_level) < 0
        or int(log_env_level) > 3
    ):
        log_env_level = "1"
...
class LOGGER(logging.Logger):
    def __init__(self, logger_name, log_level=logging.WARNING):
        ...
        self.console_log_level = (
            _name_to_log_level.get(_ms_level_to_name.get(_filter_env_level())) if log_level is None else log_level
        )
    ...
    def _get_formatter():
        ...
        """
        formatter = (
            "[%(levelname)s] %(sub_module)s(%(process)d:"
    ...
    def filter_log_str(self, msg) -> str:
        def _check_str(need_check_str):
            if len(need_check_str) > 10000:
                self.warning("Input should be <= 10000")
                return False
            filter_strs = ["\r", "\n", "\\r", "\\n"]
            for filter_str in filter_strs:
                if filter_str in need_check_str:
                    self.warning("Input should not be included \\r or \\n")
                    return False
            return True
