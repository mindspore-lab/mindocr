...
APP_ERROR DbnetPost::DbnetObjectDetectionOutput(std::vector<MxBase::Tensor> &singleResult,
...
void DbnetPost::ConstructInfo(std::vector<TextObjectInfo> &textObjectInfo, std::vector<cv::Point2f> &box,
...
void DbnetPost::FilterByMinSize(std::vector<cv::Point> &contour, std::vector<cv::Point2f> &box, float &minSide)
{
...
void DbnetPost::FilterByBoxScore(const cv::Mat &prediction, std::vector<cv::Point2f> &box, float &score)
{
...
void DbnetPost::FilterByClippedMinSize(std::vector<cv::Point2f> &box, float &minSide)
{
