...
Description: post process class of DB Net
Author: MindX SDK
Create: 2022
History: NA
"""
...
from src.utils import get_mini_boxes, unclip, construct_box, box_score_slow, \
    get_rotate_crop_image, get_hw_of_img, safe_div, box_score_fast


class DBNetPostProcess(ModuleBase):
    def __init__(self, config_path, msg_queue):
        super(DBNetPostProcess, self).__init__(config_path, msg_queue)
    ...
    def process(self, input_data):
        ...
        for box, score in zip(boxes, scores):
            ...
            infer_res_list.append(list(
                map(str, [box[1][0], box[1][1], box[2][0], box[2][1], box[3][0],
                          box[3][1], box[0][0], box[0][1]])))
