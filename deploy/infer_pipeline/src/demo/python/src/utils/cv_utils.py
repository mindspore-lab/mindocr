...
Description: utils for cv related
Author: MindX SDK
Create: 2022
History: NA
"""
...
def get_shape_from_gear(gears: list, nchw=True):
    first_gear = gears[0]
    batchsize = first_gear[0]
    channel = first_gear[1] if nchw else first_gear[3]
    if len(first_gear) != 4:
    ...
    hw_list = []
    if nchw:
        for _, _, height, width in gears:
            hw_list.append((height, width))

    else:
        for _, height, width, _ in gears:
            hw_list.append((height, width))
    return batchsize, channel, hw_list


def get_batch_from_gear(gears: list, nchw: bool = True):
    batchsize_list = []
    first_gear = gears[0]
    if len(first_gear) != 4:
        raise ValueError("model input dim must be 4")
    if nchw:
        channel, height, width = first_gear[1:]
    else:
        height, width, channel = first_gear[1:]

    for b, _, _, _ in gears:
        batchsize_list.append(b)
    return batchsize_list, channel, height, width
