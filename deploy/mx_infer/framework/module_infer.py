...
class Model:
    def __init__(self, engine_type, **kwargs):
        self.model = _INFER_ENGINE_MAP[engine_type](**kwargs)
        self.warmup()
    ...
    def get_shape_info(self):
        ...
        if channel not in (1, 3):
            raise ValueError(f"Input channel number must be 1 or 3, but got {channel} for {model_path}.")
        ...
        return ShapeType.DYNAMIC_IMAGESIZE, (batchsize, channel, tuple(sorted(hw_list)))
