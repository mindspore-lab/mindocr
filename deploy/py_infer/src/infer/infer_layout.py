...
from ..data_process.postprocess.builder import Postprocessor
from .infer_base import InferBase


class LayoutPredictor(InferBase):
    postprocess_ops: Postprocessor

    def __init__(self, args):
        super(LayoutPredictor, self).__init__(args)
    ...
    def _init_preprocess(self):
        self.preprocess_ops = build_preprocess(self.args.layout_config_path, support_gear=self.requires_gear_hw)

    def _init_model(self):
        model_path = self.args.layout_model_path
        ...
        if os.path.isdir(model_path):
            chw_list = []
            info = (
                "If --layout_model_path is folder, every model in the folder must be dynamic image_size when "
                "converting model, they must have same candidate image_size list and different batch_size. For "
                "example: NCHW format, model1_shape = (1,3,-1,-1) and model2_shape = (8,3,-1,-1), batch_size has "
                "different values of 1 and 8, and '--dynamic_dims' for image_size must have same value for every "
    ...
    def _init_postprocess(self):
        self.postprocess_ops = build_postprocess(self.args.layout_config_path)

    def get_params(self):
        return {"layout_batch_num": self._bs_list}
    ...
    def model_infer(self, data: Dict) -> List[np.ndarray]:
        net_inputs = data["net_inputs"][0]
        bs, *_ = net_inputs.shape
        n = bs if bs in self.model else -1
        return self.model[n].infer([net_inputs])

    def postprocess(self, pred: List[np.ndarray], img_shape, image_ids, ori_shape, hw_scale, pad):
        return self.postprocess_ops(pred, img_shape, meta_info=(image_ids, ori_shape, hw_scale, pad))
