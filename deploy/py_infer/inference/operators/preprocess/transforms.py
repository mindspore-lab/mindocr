...
class NormalizeImage:
    def __init__(self,
                 scale=constant.NORMALIZE_SCALE,
    ...
    def __call__(self, image: Union[np.ndarray, List[np.ndarray]]) -> np.ndarray:
        if isinstance(image, (list, tuple)):
            dst_image = [self.normalize(img, self.scale, self.std, self.mean) for img in image]
        else:
            dst_image = self.normalize(image, self.scale, self.std, self.mean)
